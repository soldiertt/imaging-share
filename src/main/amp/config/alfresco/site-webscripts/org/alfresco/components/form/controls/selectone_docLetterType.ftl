...
<div class="form-field">
   <#if form.mode == "view">
      <div class="viewmode-field">
         ...
         <span id="labelDocClass" class="viewmode-value">${valueToShow?html}</span>
      </div>
   <#else>
...
{
    url: Alfresco.constants.PROXY_URI + "be/fsoffe/standardDataList.json?type=DocLetterType",
    successCallback:
    {
       fn: onDocLetterTypeListSuccess,
       scope: this
    },
    failureCallback:
    {
       fn: onDocLetterTypeListFailed,
       scope: this
    },
    scope: this,
    noReloadOnAuthFailure: true
 });
function onDocLetterTypeListSuccess(p_response){
    ...
    for (var i = 0, len = list.length; i < len; ++i) {
		...
		{
			if("${fieldValue}" == param.id) {
				var mySpan = YAHOO.util.Dom.get("labelLetterTypeClass");
...
function onDocLetterTypeListFailed(){
	alert("Load DocLetterType list failed");
}
</script>
