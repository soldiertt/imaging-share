...
<div class="form-field">
   <#if form.mode == "view">
      <div class="viewmode-field">
         ...
         <span id="labelDocType" class="viewmode-value">${valueToShow?html}</span>
      </div>
   <#else>
...
{
    url: Alfresco.constants.PROXY_URI + "be/fsoffe/standardDataList.json?type=DocType",
    successCallback:
    {
       fn: onDocTypesListSuccess,
       scope: this
    },
    failureCallback:
    {
       fn: onDocTypesListFailed,
       scope: this
    },
    scope: this,
    noReloadOnAuthFailure: true
 });
function onDocTypesListSuccess(p_response){
    ...
    for (var i = 0, len = list.length; i < len; ++i) {
		...
		{
			if("${fieldValue}" == param.id) {
				var mySpan = YAHOO.util.Dom.get("labelDocType");
...
function onDocTypesListFailed(){
	alert("Load docTypes list failed");
}
</script>
