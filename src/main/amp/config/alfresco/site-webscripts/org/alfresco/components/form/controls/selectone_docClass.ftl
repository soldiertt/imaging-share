...
{
    url: Alfresco.constants.PROXY_URI + "be/fsoffe/standardDataList.json?type=DocClass",
    successCallback:
    {
       fn: onDocClassListSuccess,
       scope: this
    },
    failureCallback:
    {
       fn: onDocClassListFailed,
       scope: this
    },
    scope: this,
    noReloadOnAuthFailure: true
 });
function onDocClassListSuccess(p_response){
    ...
    for (var i = 0, len = list.length; i < len; ++i) {
		...
		{
			if("${fieldValue}" == param.id) {
				var mySpan = YAHOO.util.Dom.get("labelDocClass");
...
function onDocClassListFailed(){
	alert("Load DocClass list failed");
}
</script>
