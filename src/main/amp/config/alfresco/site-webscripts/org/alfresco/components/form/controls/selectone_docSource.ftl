...
<div class="form-field">
   <#if form.mode == "view">
      <div class="viewmode-field">
         ...
         <span id="labelDocSource" class="viewmode-value">${valueToShow?html}</span>
      </div>
   <#else>
...
{
    url: Alfresco.constants.PROXY_URI + "be/fsoffe/standardDataList.json?type=DocSource",
    successCallback:
    {
       fn: onDocSourceListSuccess,
       scope: this
    },
    failureCallback:
    {
       fn: onDocSourceListFailed,
       scope: this
    },
    scope: this,
    noReloadOnAuthFailure: true
 });
function onDocSourceListSuccess(p_response){
    ...
    for (var i = 0, len = list.length; i < len; ++i) {
		...
		{
			if("${fieldValue}" == param.id) {
				var mySpan = YAHOO.util.Dom.get("labelDocSource");
...
function onDocSourceListFailed(){
	alert("Load DocSource list failed");
}
</script>
