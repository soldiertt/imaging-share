...
<div class="form-field">
   <#if form.mode == "view">
      <div class="viewmode-field">
         ...
         <span id="labelDocStatus" class="viewmode-value">${valueToShow?html}</span>
      </div>
   <#else>
...
{
    url: Alfresco.constants.PROXY_URI + "be/fsoffe/standardDataList.json?type=DocStatus",
    successCallback:
    {
       fn: onDocStatusListSuccess,
       scope: this
    },
    failureCallback:
    {
       fn: onDocStatusListFailed,
       scope: this
    },
    scope: this,
    noReloadOnAuthFailure: true
 });
function onDocStatusListSuccess(p_response){
    ...
    for (var i = 0, len = list.length; i < len; ++i) {
		...
		{
			if("${fieldValue}" == param.id) {
				var mySpan = YAHOO.util.Dom.get("labelDocStatus");
...
function onDocStatusListFailed(){
	alert("Load DocStatus list failed");
}
</script>
